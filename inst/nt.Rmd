...
# Some parameters
p1 <- length(param$x)
p2 <- max(table(param$x))
p3 <- length(table(param$x))
```

It looks like you have a numeric trait. You can see a descriptive summary below:
```{r, echo = FALSE}
output <- summary(param$x)
output
```

```{r, echo = FALSE}
sk <- 3*(mean(param$x) - median(param$x))/sd(param$x)
```
...
```{r, echo = FALSE}
if(p1 > 25 & p1/p3 < 2 & p2/p1 > 0.05) table(param$x)[table(param$x)/p1 > 0.05]
```
