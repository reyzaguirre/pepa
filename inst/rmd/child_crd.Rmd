...
model <- aov(dfr[, traits[i]] ~ dfr[, geno])
...
```{r echo = FALSE}
if (at[1, 5] < 0.05) {
  agricolae::HSD.test(dfr[, traits[i]], dfr[, geno], at[2, 1], at[2, 3])$groups
} else {
  tapply(dfr[, traits[i]], dfr[, geno], mean, na.rm = TRUE)
}
```
...
```{r echo = FALSE}
if (lc$ng < 10)
  msdplot(traits[i], geno, dfr, conf = 1, xlab = 'Genotype', ylab = traits[i], pch = 4)
```
