...
lc <- ck.abd(traits[1], geno, rep, data)
```

# 1. Model specification and data description

There are data for `r lc$ng` genotypes tested using an augmented block design with `r lc$nr` blocks and `r lc$ng.check` checks in each block. The statistical model is
$$
