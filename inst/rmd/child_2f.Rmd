...
if (lc$c4 == 0)
  data$est <- mve.2f(traits[i], factors[1], factors[2], rep, design, data, maxp)[, 5] else
...
```{r, echo = FALSE}
tapply(data$est, data[, factors[1]], mean)
```

### {{i+1}}.1.2. Means by factor B levels

```{r, echo = FALSE}
tapply(data$est, data[, factors[2]], mean)
```

### {{i+1}}.1.3. Means by factor A and factor B levels

```{r, echo = FALSE}
tapply(data$est, list(data[, factors[1]], data[, factors[2]]), mean)
```
...
if (design == "crd")
  model <- aov(data[, traits[i]] ~ data[, factors[1]] * data[, factors[2]])
if (design == "rcbd")
  model <- aov(data[, traits[i]] ~ data[, factors[1]] * data[, factors[2]] + data[, rep])
par(mfrow = c(1, 2))
...
```{r, echo = FALSE}
at <- suppressWarnings(aov.2f(traits[i], factors[1], factors[2], rep, design, data, maxp))
at
```
