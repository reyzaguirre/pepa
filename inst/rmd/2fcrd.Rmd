...
data[, factors[1]] <- as.character(data[, factors[1]])
data[, factors[2]] <- as.character(data[, factors[2]])
data[, rep] <- as.character(data[, rep])
```

# 1. Model specification and data description

The data frame has two factors with `r nlevels(as.factor(data[, factors[1]]))` and `r nlevels(as.factor(data[, factors[2]]))` levels. The experimental design is a completely randomized design with `r nlevels(as.factor(data[, rep]))` replications. The statistical model is
$$
