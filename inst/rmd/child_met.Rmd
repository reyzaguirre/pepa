...
lc <- ck.f(vars[i], c(geno, env), rep, dfr)
...
if (lc$nmis > 0) {
  dfr[, y.est] <- mve.met(vars[i], geno, env, rep, dfr, maxp)[, y.est]
} else {
...
at <- suppressWarnings(aov.met(vars[i], geno, env, rep, dfr, maxp))
...
if (at[4, 5] < 0.05 & lc$nl[2] > 2) {
  ammimodel <- suppressWarnings(ammi(y.est, geno, env, rep, dfr))
...
```{r fig.align = 'center', fig.height = 8, fig.width = 8}
if (at[4, 5] < 0.05 & lc$nl[2] > 2) {
  ggemodel <- suppressWarnings(ammi(y.est, geno, env, rep, dfr, method = "gge"))
...
```{r}
if (at[4, 5] < 0.05 & lc$nl[2] > 2)
  rsamodel <- suppressWarnings(rsa(vars[i], geno, env, rep, dfr))
```
...
```{r fig.align = 'center', fig.height = 8, fig.width = 8}
if (at[4, 5] < 0.05 & lc$nl[2] > 2) {
  taimodel <- suppressWarnings(tai(y.est, geno, env, rep, dfr))
  plot(taimodel)
}
```
