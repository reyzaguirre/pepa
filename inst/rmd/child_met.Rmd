...
```{r, echo = FALSE, fig.height = 7, fig.width = 7}
if(at[4, 5] < 0.05) {
  ggemodel <- ammi(traits[i], geno, env, rep, data, method = "GGE", biplot = 1)
  ggemodel <- ammi(traits[i], geno, env, rep, data, method = "GGE", biplot = 2)
}
```
...
`r if(at[4, 5] < 0.05) {paste("### ", {{i+1}}, ".3.3. Tai stability analysis", sep = "")}`
