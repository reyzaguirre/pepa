...
```{r, echo = FALSE}
lc <- st4gi::check.met(traits[i], geno, env, rep, data)
if (lc$c3 == 0)
...
env.num <- nlevels(as.factor(data[, env]))
numrep <- nlevels(as.factor(data[, rep]))

vg <- lme4::VarCorr(model.reml)$geno[1]
vgxe <- lme4::VarCorr(model.reml)$'geno:env'[1]
vr <- attr(lme4::VarCorr(model.reml), "sc")^2

h2 <- vg / (vg + vgxe / env.num + vr / env.num / numrep) * 100
```
...
if(at[4, 5] < 0.05) {
  ammimodel <- suppressWarnings(ammi(traits[i], geno, env, rep, data, biplot = 1))
  ammimodel <- suppressWarnings(ammi(traits[i], geno, env, rep, data, biplot = 2))
}
```
...
```{r, echo = FALSE, fig.height = 7, fig.width = 7}
if(at[4, 5] < 0.05) {
  ggemodel <- suppressWarnings(ammi(traits[i], geno, env, rep, data, method = "GGE", biplot = 1))
  ggemodel <- suppressWarnings(ammi(traits[i], geno, env, rep, data, method = "GGE", biplot = 2))
}
```
