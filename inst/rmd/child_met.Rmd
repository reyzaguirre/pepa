...
if (lc$nmis > 0) {
  ...
  dfr$est <- mve.met(traits[i], geno, env, rep, dfr, maxp)[, trait.est]
} else {
  dfr$est <- dfr[, traits[i]]
}
```
...
```{r echo = FALSE}
tapply(dfr$est, dfr[, geno], mean)
```

### {{i+1}}.1.2. Means by environments

```{r echo = FALSE}
tapply(dfr$est, dfr[, env], mean)
```

### {{i+1}}.1.3. Means by genotypes and environments

```{r echo = FALSE}
tapply(dfr$est, list(dfr[, geno], dfr[, env]), mean)
```
...
```{r echo = FALSE}
if (at[1, 5] < 0.05)
  agricolae::LSD.test(dfr$est, dfr[, geno], at[5, 1], at[5, 3])$groups
```
...
```{r echo = FALSE}
if (at[1, 5] < 0.05)
  agricolae::HSD.test(dfr$est, dfr[, geno], at[5, 1], at[5, 3])$groups
```
...
if (at[4, 5] < 0.05 & lc$nl[2] > 2) {
  ammimodel <- suppressWarnings(ammi(traits[i], geno, env, rep, dfr))
...
```{r echo = FALSE, fig.align = 'center', fig.height = 8, fig.width = 8}
if (at[4, 5] < 0.05 & lc$nl[2] > 2) {
  ggemodel <- suppressWarnings(ammi(traits[i], geno, env, rep, dfr, method = "gge"))
...
```{r echo = FALSE, fig.align = 'center', fig.height = 8, fig.width = 8}
if (at[4, 5] < 0.05 & lc$nl[2] > 2) {
  taimodel <- suppressWarnings(tai(traits[i], geno, env, rep, dfr))
  plot(taimodel)
}
```
