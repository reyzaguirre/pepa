...
data <- params$data
data <- data[, c("PHASE", "IDENTIFIED_CRITERIA", "SCORE_MEN", "SCORE_WOMEN", "SCORE_GLOBAL")]
data$IDENTIFIED_CRITERIA <- as.character(data$IDENTIFIED_CRITERIA)
colnames(data) <- c("phase", "Criteria", "Men", "Women", "Global")
data <- tidyr::gather(data, group, value, Men:Global)

## Split by phase

flow <- data[data$phase == "Flowering", ]
harv <- data[data$phase == "Harvest", ]
stor <- data[data$phase == "Storage", ]
