...
if (lc$nmis > 0) {
  dfr[, y.est] <- mve.rcbd(vars[i], trt, rep, dfr, maxp)[, y.est]
} else {
...
at <- suppressWarnings(aov.rcbd(vars[i], trt, rep, dfr, maxp))
...
```{r fig.height = 5, fig.width = 10}
if (lc$ng < 10) msdplot(vars[i], trt, dfr, conf = 1, pch = 4)
```
