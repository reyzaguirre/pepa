...
```{r, echo = FALSE}
lc <- check.rcbd(traits[i], treat, rep, data)
at <- suppressWarnings(rcbd(traits[i], treat, rep, data, maxp))
...
Don't forget the assumptions of the model. It is supposed that the errors are independent with a normal distribution and with the same variance for all the treatments. The following plots must help you evaluate this:
...
```{r, echo = FALSE}
if (at[1, 5] < 0.05)
  agricolae::HSD.test(data[, traits[i]], data[, treat], at[3, 1], at[3, 3])$groups else
...
Below are the variance components for this model, under the assumption that treatments and blocks are random. Here the model is fitted using REML and missing values are not estimated.

```{r, echo = FALSE}
ff <- as.formula(paste(traits[i], "~ (1|", treat, ") + (1|", rep, ")"))
