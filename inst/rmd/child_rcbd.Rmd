...
`r if(at[1, 5] < 0.05) {"Below are the sorted means for each genotype with letters indicating if there are significant differences using the least significance difference method and the multiple comparisons method of Tukey, both at the 5% level."} else {"The means of your genotypes are:"}`

`r if (at[1, 5] < 0.05) {paste("### ", {{i+1}}, ".1. LSD test", sep = "")}`
...
`r if (at[1, 5] < 0.05) {paste("### ", {{i+1}}, ".2. Tukey test", sep = "")}`
...
`r if (lc$nt < 10 & at[1, 5] < 0.05) {paste("### ", {{i+1}}, ".3. Plot of means", sep = "")}`
